function sbxalignnplanedir_1000(chan,varargin)
    ...
    try
        fn = strtok(d(i).name,'.');
         if ~ exist([fn '.align'])
            ...
            T=zeros(500+1,2);%%%%CAN'T be uint16!!!
            if  info.nchan >1 
                if  chan ==0
                chan = menu('Use which channel to align?','Green','Red')
                end
                for i=1:nplanes
                    [m(:,:,i),mg(:,:,i),mr(:,:,i),T(i:nplanes:500+1,:)] = sbxalignx2(fn,i-1:nplanes:500,chan);
                end
                save([fn '.align'],'m','mg','mr','T');
                clear m mg mr T;
            else
                for i=1:nplanes
                    [m(:,:,i),T(i:nplanes:500+1,:)] = sbxalignx(fn,i-1:nplanes:500);
                end
                save([fn '.align'],'m','T');
                clear m T;
                display(sprintf('Done %s: Aligned %d images in %d min',fn,500+1,round(toc/60)));
            end
        else
