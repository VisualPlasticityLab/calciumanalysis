 function caanalysisplot_sp(varargin)
if nargin<1
...
%save([fname '_' num2str(numel(Cor)) 'signals.mat'],'sigspF','matrix','win_sig','Cor');
if ~exist('matrix','var') 
    matrix = run.matrix;
end
...
if ~exist('sigspF','var')
    ...
    try
        sigspF = SI.sigspF;
        SI = rmfield(SI,'sigspF');
    catch
        [file,path]=uigetfile('*signals.mat','select sigspF file');
        signals=load(fullfile(path,file))
        matrix = signals.matrix;
        win_sig = signals.window;
        sigspF= signals.sigspF;
    end
end
 
sigspF(isnan(sigspF))=0;
...
switch drawingoption
    case 0%'no running with contrast map'
        hsigspF=sigFplt(sigspF,[],win_sig,Cor);  % sigspF:seg,rep,Var,ncell
        [SI.peak,~,SI.error]=cal_ER(sigspF,win_sig);%peak: 1*Var*ncell
...
%         hsigspF=sigFplt(sigspF,[],win_sig,Cor);  % sigspF:seg,rep,Var,ncell
    ...
    case 2 %'no running with both contrast&orientation map'
        %[hsigspF,Gd]=sigVarplt(sigspF,win_sig,[],Cor);  % sigspF:seg,rep,Var,ncell
        [SI.peak,~,SI.error]=cal_ER(SI.sigspF,win_sig);%peak:1*Var*ncell
    ...
    case 10  %'running with contrast'
        [~,SI.peakR,SI.errorR,~,SI.peakS,SI.errorS]= sigFcmp(sigspF,win_sig,matrix);  % sigR:seg,1,Var,ncell
        hsigspF=sigFplt(sigspF,matrix,win_sig,Cor);  % sigspF:seg,rep,Var,ncell
...
%         SI.peakS=fixpeak(SI.peakS);
    ...
    case 12%'running with both contrast and orientation'
        hsigspF=sigVarplt(sigspF,win_sig,matrix,Cor);  % sigspF:seg,rep,Var,ncell
        [~,SI.peakR,SI.errorR,~,SI.peakS,SI.errorS]= sigFcmp(sigspF,win_sig,matrix);  % calculate peak from prestim:prestim+stimON-1
...
%     baseline=nanmean(sigspF(baselinewin,:,:,:));
%     seg=size(sigspF,1);
%     sigspF=sigspF-repmat(baseline,[seg 1 1 1 ]);
%     threshold=2*nanstd(sigspF(baselinewin,:,:,:));% threshold size: 1,1,Var,ncell
%     sigspF(repmat(mean(sigspF(win_sig(1):win_sig(end),:,:,:))<threshold,[seg 1 1 1 ]))=NaN; % set
%     [b,a] = butter(12,.2,'low');           % IIR filter design
%     sigspF = filtfilt(b,a,sigspF);
    save(fullfile(folder,'peakSI.mat'),'SI','sigspF','variant','Cor','matrix','win_sig');
    ...
    try
        ...
        for i=1:numel(hsigspF)
        saveas(hsigspF(i),fullfile(folder,hsigspF(i).Name),'png');
        end
    end
 end
close all
