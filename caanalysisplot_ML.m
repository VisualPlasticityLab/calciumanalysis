 function caanalysisplot_ML(varargin)
if nargin<1
...
folder = fullfile(folder,'sp')
if ~exist(folder)
    mkdir(folder)
end
%% plot figures and calculate Orientation selection or Contrast invariant
%save([fname '_' num2str(numel(Cor)) 'signals.mat'],'sigMLF','matrix','win_sig','Cor');
if ~exist('matrix','var') 
    matrix = run.matrix;
end
...
if ~exist('sigMLF','var')
    ...
    try
        sigMLF = SI.sigMLF;
        SI = rmfield(SI,'sigMLF');
    catch
        [file,path]=uigetfile('*signals.mat','select sigMLF file');
        signals=load(fullfile(path,file))
        matrix = signals.matrix;
        win_sig = signals.window;
        sigMLF= signals.sigMLF;
    end
end
 
sigMLF(isnan(sigMLF))=0;
...
switch drawingoption
    case 0%'no running with contrast map'
        hsigMLF=sigFplt(sigMLF,[],win_sig,Cor);  % sigMLF:seg,rep,Var,ncell
        [SI.peak,~,SI.error]=cal_ER(sigMLF,win_sig);%peak: 1*Var*ncell
...
%         hsigMLF=sigFplt(sigMLF,[],win_sig,Cor);  % sigMLF:seg,rep,Var,ncell
    ...
    case 2 %'no running with both contrast&orientation map'
        %[hsigMLF,Gd]=sigVarplt(sigMLF,win_sig,[],Cor);  % sigMLF:seg,rep,Var,ncell
        [SI.peak,~,SI.error]=cal_ER(SI.sigMLF,win_sig);%peak:1*Var*ncell
    ...
    case 10  %'running with contrast'
        [~,SI.peakR,SI.errorR,~,SI.peakS,SI.errorS]= sigFcmp(sigMLF,win_sig,matrix);  % sigR:seg,1,Var,ncell
        hsigMLF=sigFplt(sigMLF,matrix,win_sig,Cor);  % sigMLF:seg,rep,Var,ncell
...
%         SI.peakS=fixpeak(SI.peakS);
    ...
    case 12%'running with both contrast and orientation'
        hsigMLF=sigVarplt(sigMLF,win_sig,matrix,Cor);  % sigMLF:seg,rep,Var,ncell
        [~,SI.peakR,SI.errorR,~,SI.peakS,SI.errorS]= sigFcmp(sigMLF,win_sig,matrix);  % calculate peak from prestim:prestim+stimON-1
...
%     baseline=nanmean(sigMLF(baselinewin,:,:,:));
%     seg=size(sigMLF,1);
%     sigMLF=sigMLF-repmat(baseline,[seg 1 1 1 ]);
%     threshold=2*nanstd(sigMLF(baselinewin,:,:,:));% threshold size: 1,1,Var,ncell
%     sigMLF(repmat(mean(sigMLF(win_sig(1):win_sig(end),:,:,:))<threshold,[seg 1 1 1 ]))=NaN; % set
%     [b,a] = butter(12,.2,'low');           % IIR filter design
%     sigMLF = filtfilt(b,a,sigMLF);
    save(fullfile(folder,'peakSI.mat'),'SI','sigMLF','variant','Cor','matrix','win_sig');
    ...
    try
        ...
        for i=1:numel(hsigMLF)
        saveas(hsigMLF(i),fullfile(folder,hsigMLF(i).Name),'png');
        end
    end
 end
close all
